...
Host website to keep track of all listened songs, lowk a discord bot would also be easy
...
class Cleaner:
    ...
    def save_queue(self):
        ...
        if not cp:
            ...
            return
        position, volume = cp['progress_ms'], cp['device']['volume_percent']
        ...
        print(f"Queue saved to '{pname}'\nAdd that playlist to queue to get it back")
        return pname
    ...
    def check_playlist_for_duplicates(self, playlist):
        ...
        dupes, seen, t_counts = [], set(), Counter(t for *_, t in pl_tracks)
        for i, a, t in pl_tracks:
            if t_counts[t] > 1:
                if (a, t) not in seen:
                    seen.add((a, t))
   ...
   # Helper
    ...
    # Helper
    def create_pl(self, playlist_name, ids=None):
        """
        ...
        for id, name in self.get_my_playlists(only_mine=True):
            if name == playlist_name:
                ...
                return id if input(s).lower() == 'y' else None
